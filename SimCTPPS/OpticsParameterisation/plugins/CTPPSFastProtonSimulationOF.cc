...
class CTPPSFastProtonSimulationOF : public edm::stream::EDProducer<>
{
  public:
    explicit CTPPSFastProtonSimulationOF( const edm::ParameterSet& );
    ~CTPPSFastProtonSimulationOF();
  ...
  private:
    struct CTPPSPotInfo
    {
      ...
      CTPPSPotInfo( const CTPPSDetId& det_id, double z_position, LHCOpticsApproximatorOF* approx, double _x_max ) :
        detid( det_id ), z_position( z_position ), approximator( approx ), x_max(_x_max) {}

      CTPPSDetId detid;
      double z_position;
      LHCOpticsApproximatorOF* approximator;
      double x_max;
    };
...
const bool CTPPSFastProtonSimulationOF::invertBeamCoordinatesSystem_ = true;

//----------------------------------------------------------------------------------------------------

CTPPSFastProtonSimulationOF::CTPPSFastProtonSimulationOF( const edm::ParameterSet& iConfig ) :
...
CTPPSFastProtonSimulationOF::~CTPPSFastProtonSimulationOF()
{}


//----------------------------------------------------------------------------------------------------

void CTPPSFastProtonSimulationOF::beginRun( const edm::Run&, const edm::EventSetup& iSetup )
{
}

//----------------------------------------------------------------------------------------------------

void CTPPSFastProtonSimulationOF::produce( edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  // get input
...
void CTPPSFastProtonSimulationOF::processProton(const HepMC::GenVertex* in_vtx, const HepMC::GenParticle* in_trk,
  const CTPPSGeometry &geometry, std::vector<CTPPSLocalTrackLite> &out_tracks, edm::DetSetVector<TotemRPRecHit>& out_strip_hits,
  edm::DetSetVector<CTPPSPixelRecHit> &out_pixel_hits, edm::DetSetVector<CTPPSDiamondRecHit> &out_diamond_hits) const
{
  ...
  for (const auto& rp : pots_)
  {
    ...
    // transport proton
    double kin_tr_out[5];
    rp.approximator->Transport(kin_tr_in, kin_tr_out);
    const double b_x_tr = kin_tr_out[0], b_y_tr = kin_tr_out[2];
    const double a_x_tr = kin_tr_out[1], a_y_tr = kin_tr_out[3];

    // determine beam position
    double kin_be_in[5] = { 0., 0., vtx_y_offset, 0., 0. };
    double kin_be_out[5];
    rp.approximator->Transport(kin_be_in, kin_be_out);
...
void
CTPPSFastProtonSimulationOF::fillDescriptions( edm::ConfigurationDescriptions& descriptions )
{
...
DEFINE_FWK_MODULE( CTPPSFastProtonSimulationOF );
