...
class CTPPSProtonReconstructionOF : public edm::stream::EDProducer<>
{
  public:
    explicit CTPPSProtonReconstructionOF(const edm::ParameterSet&);
...
CTPPSProtonReconstructionOF::CTPPSProtonReconstructionOF( const edm::ParameterSet& iConfig ) :
  ...
  if (applyExperimentalAlignment)
  {
    for (const auto &f : alignmentFiles_)
    {
      ...
      if (alignmentCollection_.Load(fip.fullPath()) != 0)
        throw cms::Exception("CTPPSProtonReconstructionOF") << "Cannot load alignment from file '" << f << "'.";
    }
  }
...
void CTPPSProtonReconstructionOF::fillDescriptions(ConfigurationDescriptions& descriptions)
{
...
CTPPSProtonReconstructionOF::~CTPPSProtonReconstructionOF()
{
}

//----------------------------------------------------------------------------------------------------

void CTPPSProtonReconstructionOF::produce(Event& event, const EventSetup&)
{
  // get input
...
DEFINE_FWK_MODULE(CTPPSProtonReconstructionOF);
