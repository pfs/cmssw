...
#ifndef RecoCTPPS_ProtonReconstruction_ProtonReconstructionAlgorithmOF_h
#define RecoCTPPS_ProtonReconstruction_ProtonReconstructionAlgorithmOF_h
...
class ProtonReconstructionAlgorithmOF
{
  public:
    ProtonReconstructionAlgorithmOF(const std::string &optics_file_beam1, const std::string &optics_file_beam2,
      const edm::ParameterSet &beam_conditions, const std::vector<edm::ParameterSet> &detector_packages, unsigned int _verbosity);

    ~ProtonReconstructionAlgorithmOF();
  ...
  private:
    ...
    struct RPOpticsData
    {
      std::shared_ptr<LHCOpticsApproximatorOF> optics;
